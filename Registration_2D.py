...
import os
import time
from Network import BrainNet, Net2D
from Loss import *
...
def main(config):
    ...
    img_size = 256
    fixed = data[100:100+img_size,50:50+img_size,1,5]
    ...
    t = time.time()
    df, df_with_grid, warped_moving = registration2d(config, device, moving, fixed)
    runtime = time.time() - t
    print('Registration Running Time:', runtime)
    print('---Registration DONE---')
    #evaluation(config, device, df, df_with_grid)
    print('---Evaluation DONE---')
    save_result(config, df,df_with_grid, warped_moving)
    print('---Results Saved---')


def registration2d(config, device, moving, fixed):
    '''
    ...
    '''
    im_shape = fixed.shape[:2]
    print(im_shape)
    moving = torch.from_numpy(moving.astype('float')).float().to(device)
    fixed = torch.from_numpy(fixed.astype('float')).float().to(device)
    #moving = torch.from_numpy(moving).to(device).float()[:,:,80]
    #fixed = torch.from_numpy(fixed).to(device).float()[:,:,80]
    # make batch dimension - why up to 5 dimensions? - B, C , H , W, D
    moving = moving.unsqueeze(0).unsqueeze(0)
    fixed = fixed.unsqueeze(0).unsqueeze(0)
    #print(moving.shape)
    Network = Net2D(img_sz=im_shape,
    ...
    loss_NCC = NCC_vxm(win=config.NCC_win)
    ...
    for i in range(config.epoches):
        ...
        # similarity loss
        loss_sim = loss_NCC.loss(warped_moving, fixed)
...
def save_result(config, df, df_with_grid,warped_moving):
    save_nii(df.permute(2,3,0,1).detach().cpu().numpy(), '%s/df.nii.gz' % (config.savepath))
    save_nii(df.permute(2,3,0,1).detach().cpu().numpy(), '%s/df_grid.nii.gz' % (config.savepath))
